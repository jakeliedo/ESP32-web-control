...
void setup_wifi() {
  delay(10);
  Serial.print("[wc3] Connecting to WiFi");
...
void publish_status() {
  ...
  Serial.println("[wc3] Status published");
}
...
void publish_response(const char* action, bool success, const char* message) {
  ...
  client.publish(topic.c_str(), buf, n);
  Serial.print("[wc3] Response published: ");
  Serial.println(action);
}

void blink_led_4s() {
  Serial.println("[wc3] Blinking LED for 4 seconds...");
  for (int i = 0; i < 20; i++) {
  ...
  digitalWrite(LED_PIN, HIGH);
  Serial.println("[wc3] LED blinking done");
}
...
void stop_relay() {
  ...
  Serial.println("[wc3] Relay stopped");
}
...
void execute_flush() {
  ...
  Serial.println("[wc3] FLUSH command received!");
  blink_led_4s();
}
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  String action = msg;
  Serial.print("[wc3] MQTT message received: ");
  Serial.println(msg);
  // Thử parse JSON
  ...
  if (!err) {
    ...
    Serial.print("[wc3] Parsed action: ");
    Serial.println(action);
  }
  action.trim();
  if (action == "flush" || action == "on" || action == "activate") {
    ...
    publish_status();
    Serial.println("[wc3] STATUS command received!");
  } else if (action == "off" || action == "stop") {
    stop_relay();
    Serial.println("[wc3] STOP command received!");
  } else {
    publish_response(action.c_str(), false, "Unknown action");
    Serial.print("[wc3] Unknown action: ");
    Serial.println(action);
  }
}
...
void reconnect() {
  while (!client.connected()) {
    Serial.println("[wc3] Attempting MQTT connection...");
    if (client.connect(node_id)) {
      String topic = String("wc/") + node_id + "/command";
      client.subscribe(topic.c_str());
      Serial.print("[wc3] Subscribed to topic: ");
      Serial.println(topic);
    } else {
      Serial.println("[wc3] MQTT connection failed, retrying...");
      delay(2000);
    }
  }
}
...
void setup() {
  Serial.begin(115200);
  delay(100);
  Serial.println("[wc3] Starting node...");
  pinMode(RELAY_PIN, OUTPUT);
  pinMode(LED_PIN, OUTPUT);
  digitalWrite(RELAY_PIN, LOW);
  digitalWrite(LED_PIN, HIGH);
  setup_wifi();
  Serial.println("[wc3] WiFi connected!");
  client.setServer(mqtt_server, 1883);
  client.setCallback(callback);
  // Test LED startup
  Serial.println("[wc3] Testing LED...");
  for (int i = 0; i < 3; i++) {
    digitalWrite(LED_PIN, LOW); delay(300);
    digitalWrite(LED_PIN, HIGH); delay(300);
  }
  Serial.println("[wc3] LED test complete");
  publish_status();
  Serial.println("[wc3] Node ready!");
}
